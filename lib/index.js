...
class PuppeteerHTMLPDF {
  ...
  async initializeBrowser() {
    ...
    try {
      if (this.options && this.options.browserWSEndpoint) {
        ...
        const headless = (this.options && has(this.options, 'headless')) 
  ...
  setPageHeaders(page) {
    if (this.options && this.options.authorization) {
  ...
  async generatePDF(page) {
    const data = await page.pdf({
      ...omit(this.options, ['authorization', 'executablePath', 'args', 'headless']),
      printBackground: has(this.options, 'printBackground') 
